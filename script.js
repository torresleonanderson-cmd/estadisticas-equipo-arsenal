...
// --- ESTRUCTURA DE DATOS ---
let plantillaCompleta = []; // Aquí vivirá la plantilla maestra de la BD
let jugadoresConvocados = []; // Jugadores solo para el partido actual
let historialDePartidos = []; // Historial de partidos de la BD
...
async function cargarDatosDesdeLaNube() {
    try {
        ...
        // Cargamos el próximo partido guardado localmente (esto no necesita estar en la nube)
...
async function finalizarPartido() {
    ...
    const partido = {
        fecha: new Date().toISOString().split('T')[0], // La fecha de hoy
    ...
    try {
        ...
        // Limpiamos todo para el siguiente partido
        jugadoresConvocados = [];
        ...
        // Mostramos un mensaje de éxito y recargamos los datos
...
function convocarJugador() {
    ...
    // Añadimos al jugador a la lista de convocados con stats de partido en cero
...
// --- ACTUALIZACIÓN DE VISTA (MODIFICADO) ---
...
function actualizarTablaLideres() {
    const lideres = [...plantillaCompleta].sort((a, b) => b.goles_totales - a.goles_totales || b.asistencias_totales - a.asistencias_totales);
    const tbody = document.querySelector('#tabla-lideres tbody');
    tbody.innerHTML = '';
    lideres.slice(0, 10).forEach(j => { // Mostramos solo el top 10
        const fila = document.createElement('tr');
        fila.innerHTML = `<td>${j.nombre}</td><td>${j.goles_totales}</td><td>${j.asistencias_totales}</td>`;
...
function actualizarTablaJugadoresConvocados() {
    ...
    jugadoresConvocados.forEach((j, i) => {
        const fila = document.createElement('tr');
        fila.innerHTML = `
            <td>${iconosPosicion[j.posicion] || ''}</td>
            <td>${j.nombre}</td>
            <td><div class="stat-controls"><button class="stat-btn" onclick="actualizarEstadistica(${i},'goles','restar')">-</button><span class="stat-value">${j.statsPartido.goles}</span><button class="stat-btn" onclick="actualizarEstadistica(${i},'goles','sumar')">+</button></div></td>
            <td><div class="stat-controls"><button class="stat-btn" onclick="actualizarEstadistica(${i},'asistencias','restar')">-</button><span class="stat-value">${j.statsPartido.asistencias}</span><button class="stat-btn" onclick="actualizarEstadistica(${i},'asistencias','sumar')">+</button></div></td>
            <td><div class="stat-controls"><button class="stat-btn" onclick="actualizarEstadistica(${i},'amarillas','restar')">-</button><span class="stat-value">${j.statsPartido.amarillas}</span><button class="stat-btn" onclick="actualizarEstadistica(${i},'amarillas','sumar')">+</button></div></td>
            <td><div class="stat-controls"><button class="stat-btn" onclick="actualizarEstadistica(${i},'rojas','restar')">-</button><span class="stat-value">${j.statsPartido.rojas}</span><button class="stat-btn" onclick="actualizarEstadistica(${i},'rojas','sumar')">+</button></div></td>
            <td><button class="btn-eliminar" onclick="abrirModalConfirmacion(${i},'${j.nombre.replace(/'/g, "\\'")}')"><i class="fa-solid fa-trash"></i></button></td>
        `;
        tbody.appendChild(fila);
    });
}
...
// --- FUNCIONES QUE NO CAMBIAN MUCHO ---
function cambiarVista(vistaActivaId) { document.getElementById('vista-partido').classList.toggle('vista-oculta'); document.getElementById('vista-estadisticas').classList.toggle('vista-oculta'); document.getElementById('btn-vista-partido').classList.toggle('active'); document.getElementById('btn-vista-estadisticas').classList.toggle('active'); if (vistaActivaId === 'vista-estadisticas') { actualizarVista(); } }
...
// La función de Reset Total ahora la dejaremos pendiente, ya que borrar de la BD es más delicado
function abrirModalReset() {
    mostrarAlerta('Función Deshabilitada', 'La opción de reinicio total está en mantenimiento para proteger los datos de la nube.');
}